...
@app.get("/health")
def health():
    return jsonify({"ok": True, "service": "Samuel AI backend"})
...
def build_clarification(filters, matches):
    if not filters.get("category") and not filters.get("product"):
        return True, "What are you shopping for? You can say men's clothing, women's clothing, electronics, or jewelry.", ["category_or_product"]
    ...
    if matches is not None and len(matches) == 0:
        return True, "I didn’t find an exact match — would you like me to show similar items?", ["category_or_product_or_price"]
    return False, "", []

# --- MAIN ENDPOINT: /parse-voice -------------------------------------
...
def parse_voice():
    ...
    action: "add_to_cart" | "remove_from_cart" | "filter"
    product: short keyword/title fragment
...
@app.post("/speak")
def speak_text():
    """Converts text to speech and returns audio/mpeg."""
    try:
        data = request.get_json(silent=True) or {}
        text = data.get("text", "")
        speaker = data.get("speaker", "Samuel").lower()  # Default to Samuel
        ...
        if speaker == "sandy":
            ...
            voice = "alloy"  # Male voice for Samuel or default
        ...
        audio_stream = io.BytesIO(speech.read())
        audio_stream.seek(0)
